...
const UserIncomeHistory = () => {
  ...
  const columns = [
    ...
    // },
    {
      ...
      cell: (row) => (
        <span className="font-medium text-white">
          {maskWalletAddress(row?.clientAddress || row?.fromUser|| row?.tx?.clientAddress) || "-"}
        </span>
      ),
    },
    // {
    ...
    //   cell: (row) => (
    //     <span className="font-medium text-white">
    //       {maskWalletAddress(row?.mainAddress || row?.tx?.mainAddress) || "-"}
    //     </span>
    //   ),
    // },
    {
      ...
      cell: (row) => (
        <span className="font-medium text-white">
  ${Number(row?.investment || row?.amount || row?.tx?.investment || 0).toFixed(3)}
</span>
      ),
    },
    {
      ...
      cell: (row) => (
       <span className="font-medium text-white">
  {row?.percentage ? Number(row.percentage).toFixed(2) : "0.00"}%
</span>
      ),
    },
    // {
    ...
    //   cell: (row) => (
    //     <span className="font-medium text-white">
    //       {row?.package?.title || row?.package?._id || "-"}
    //     </span>
    //   ),
    //   searchValue: (row) => row?.package?.title || row?.package?._id,
    // },
    ...
    //   accessor: "hash",
    //   cell: (row) => (
    //     <span className="font-medium text-white">{row?.hash || row?.id || row?.tx?.hash || "-"}</span>
    //   ),
    // },
  ];
  ...
  const cardData = [
      ...
      //   value: `$ ${Number(totalIncome?.totalLevelIncome ?? 0).toFixed(2)}`,
      ...
      // },
      // {
      //   title: "Direct Referrals Income",
      //   value: `$ ${Number(totalIncome?.directReferralIncome ?? 0).toFixed(2)}`,
