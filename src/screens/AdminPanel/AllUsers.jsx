    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-col">
          {row?.sponsor ? (
            <>
              <span className="font-medium text-blue-400">{row?.sponsor?.id || 'N/A'}</span>
              <span className="text-xs text-slate-400">{row?.sponsor?.username || ''}</span>
            </>
          ) : (
            <span className="font-medium text-slate-500">No Sponsor</span>
          )}
        </div>
      ),
      searchValue: (row) => row?.sponsor?.id || row?.sponsor?.username || 'no sponsor',
    },
    {
      header: "Investment",
      accessor: "investment",
