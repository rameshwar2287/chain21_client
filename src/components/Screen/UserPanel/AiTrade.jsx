...
import { Search, TrendingUp, Clock, Bot, Cpu, History } from "lucide-react";
...
const AiTrade = () => {
  ...
  const filteredIncomeHistory = useMemo(() => {
    return data === "today"
      ? allTradingIncomeHistory.filter((item) => isToday(new Date(item.createdAt)))
      : allTradingIncomeHistory;
  }, [allTradingIncomeHistory, data]);

  const columns = [
    {
      header: "S/N",
      accessor: "_id",
      cell: (row, rowIndex) => <span className="text-gray-400">{rowIndex + 1}</span>,
    },
    {
      header: "User ID",
      accessor: "id",
      cell: (row) => <span className="text-white font-medium">{row?.user?.username || "N/A"}</span>,
      searchValue: (row) => row?.user?.username
    },
    {
      header: "Package",
      accessor: "package.title",
      cell: (row) => <span className="text-[#FFD700] font-bold">{row?.package?.title || "N/A"}</span>,
      searchValue: (row) => row?.package?.title
    },
    {
      header: "Amount",
      accessor: "amount",
      cell: (row) => <span className="text-white font-mono">$ {row?.amount}</span>,
    },
    {
      header: "Income",
      accessor: "income",
      cell: (row) => <span className="text-green-400 font-bold">+$ {row?.income.toFixed(2)}</span>,
    },
    {
      header: "Status",
      accessor: "status",
      cell: (row) => (
        <span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${
          row?.status === "Completed" ? "bg-green-500/10 text-green-500" : "bg-[#B8860B]/10 text-[#FFD700]"
        }`}>
          {row?.status}
        </span>
      ),
    },
    {
      header: "Timestamp",
      accessor: "createdAt",
      cell: (row) => <span className="text-gray-500 text-xs">{new Date(row?.createdAt).toLocaleString()}</span>,
    },
  ];
  ...
  const fetchAllCoins = async () => {
    try {
      const data = await fetchCoinMarkets();
      setAllCoins(data);
      if (!selectedCoin) setSelectedCoin(data?.[0]);
  ...
  return (
    <div className="space-y-8 p-4 lg:p-0 bg-black min-h-screen">
      
      {/* Header */}
      <div className="flex items-center gap-4 pt-8">
        <div className="p-3 rounded-2xl bg-[#B8860B]/10 border border-[#B8860B]/30 text-[#FFD700]">
          <Bot size={32} />
        </div>
        <div>
          <h1 className="text-3xl font-bold font-rajdhani text-white uppercase tracking-wider">
            AI TRADE <span className="hero-text-gradient">ENGINE</span>
          </h1>
          <p className="text-gray-500 text-sm font-poppins uppercase tracking-widest">Autonomous ROI Generation Protocol</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        {/* Chart & Action */}
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[#0a0a0a] border border-[#2a2a2a] rounded-3xl p-6 shadow-2xl relative overflow-hidden">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
              {selectedCoin && (
                <div className="flex items-center gap-4">
                  <img src={selectedCoin.image} alt={selectedCoin.name} className="w-10 h-10 rounded-full border border-[#FFD700]/30 p-1" />
                  <div>
                    <h2 className="text-2xl font-bold font-rajdhani text-white">
                      {selectedCoin.name} <span className="text-[#FFD700] opacity-60">/{selectedCoin.symbol.toUpperCase()}</span>
                    </h2>
                  </div>
                </div>
              )}
              <div className="flex gap-1 bg-black p-1 rounded-xl border border-[#2a2a2a]">
                {["24H", "7D", "1M", "3M"].map((label, idx) => (
                  <button key={label} onClick={() => setTimeframe(["1","7","30","90"][idx])} className={`px-4 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${timeframe === ["1","7","30","90"][idx] ? "bg-gradient-to-r from-[#B8860B] to-[#FFD700] text-black" : "text-gray-500 hover:text-white"}`}>
                    {label}
                  </button>
                ))}
              </div>
          ...
          {/* AI TRADE BUTTON - HIGH VISIBILITY FIX */}
          <motion.button
            whileHover={!isButtonDisabled ? { scale: 1.01, shadow: "0 0 40px rgba(212, 175, 55, 0.4)" } : {}}
            whileTap={!isButtonDisabled ? { scale: 0.99 } : {}}
            onClick={handleAiTradeClick}
            disabled={isButtonDisabled}
            className={`relative flex items-center justify-center gap-4 w-full p-6 rounded-2xl font-black font-rajdhani text-xl uppercase tracking-widest transition-all duration-500 shadow-2xl overflow-hidden border-2 ${
              isButtonDisabled
                ? "bg-[#111] border-[#2a2a2a] text-gray-600 cursor-not-allowed"
                : "bg-gradient-to-r from-[#B8860B] to-[#FFD700] border-transparent text-black"
            }`}
          >
            {isButtonDisabled ? (
              <>
                <Clock className="w-7 h-7 animate-spin" />
                <span>PROTOCOL COOLDOWN: {formatCountdown(countdownTime)}</span>
              </>
            ) : (
              <>
                <Cpu className="w-7 h-7" />
                <span>EXECUTE AI TRADE ENGINE</span>
        ...
        {/* Assets Sidebar */}
        <div className="lg:col-span-1 bg-[#0a0a0a] border border-[#2a2a2a] rounded-3xl p-5 flex flex-col h-[610px] shadow-2xl">
          <h3 className="text-white font-bold font-rajdhani uppercase tracking-widest mb-6 px-2 flex items-center gap-2">
            <TrendingUp size={18} className="text-[#FFD700]" /> Global Assets
          </h3>
          <div className="relative mb-6">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600" size={18} />
            <input type="text" placeholder="Search assets..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-black border border-[#2a2a2a] rounded-xl py-3 pl-12 pr-4 text-white text-sm focus:border-[#FFD700] outline-none" />
          </div>
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2">
            {filteredCoins?.map((coin) => (
              <CoinListItem key={coin.id} coin={coin} onSelect={setSelectedCoin} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 pb-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-2xl font-bold font-rajdhani text-white uppercase tracking-wider">Trading <span className="hero-text-gradient">Ledger</span></h2>
        </div>
        <DataTable title="Profit Distributions" columns={columns} data={filteredIncomeHistory} pageSize={10} />
      </div>
    </div>
  );
};
